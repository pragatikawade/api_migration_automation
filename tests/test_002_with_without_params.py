...
def test_002_with_without_params(mulesoft_basic_auth_header, azure_basic_auth_header, api_config):
    ...
    # MuleSoft
    # -------------------------------
    mule_resp, mule_time = timed_request("GET", mule_url, mulesoft_basic_auth_header)
    logger.info(f"MuleSoft → Status: {mule_resp.status_code} | Time: {mule_time:.3f}s | Authenticated: {mule_resp.json().get('authenticated')}")

    # -------------------------------
    # Azure
    # -------------------------------
    azure_resp, azure_time = timed_request("GET", azure_url, azure_basic_auth_header)
