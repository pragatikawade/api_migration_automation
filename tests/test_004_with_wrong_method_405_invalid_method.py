...
def test_003_with_invalid_url_endpoint_404_not_found(mulesoft_basic_auth_header, azure_basic_auth_header, api_config):
    ...
    # MuleSoft
    # -------------------------------
    mule_resp, mule_time = timed_request("POST", mule_url, mulesoft_basic_auth_header, params)
    try:
        if "application/json" in mule_resp.headers.get("Content-Type", ""):
            mule_data = mule_resp.json()
        else:
            mule_data = mule_resp.text  # raw text fallback
    except Exception:
        mule_data = mule_resp.text  # fallback in case headers are missing
    logger.info(f"MuleSoft → Status: {mule_resp.status_code} | Time: {mule_time:.3f}s | Response: {mule_data}")

    # -------------------------------
    # Azure
    # -------------------------------
    azure_resp, azure_time = timed_request("POST", azure_url, azure_basic_auth_header, params)
    try:
        if "application/json" in azure_resp.headers.get("Content-Type", ""):
           azure_data = azure_resp.json()
        else:
            azure_data = azure_resp.text  # raw text fallback
    except Exception:
        azure_data = azure_resp.text  # fallback in case headers are missing
    logger.info(f"MuleSoft → Status: {azure_resp.status_code} | Time: {azure_time:.3f}s | Response: {azure_data}")
